...
export const eventController = {
  ...
  getAll: async (req: Request, res: Response) => {
    try {
      ...
      // Transform response để frontend có thể dùng current_participants
      const transformedEvents = events.map(event => ({
        ...event,
        current_participants: event._count.registrations,
        like_count: event._count.likes
      }))
    ...
    try {
      ...
      const registration = await prismaService.client.event_registrations.create({
        data: {
          ...
          student_phone: registrationData.student_phone || null,
          major: registrationData.major || null,
          semester: registrationData.semester || null,
          class_code: registrationData.class_code || null,
          notes: registrationData.notes || null,
          status: 'registered'
        }
      })
      
      // Đếm lại số registrations để đảm bảo chính xác
      const registrationCount = await prismaService.client.event_registrations.count({
        where: { event_id: parseInt(id) }
      })
      
      // Update current participants count với số thực tế
